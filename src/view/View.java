...
public class View<T extends Taskable> {
	...
	public View(Journal<? extends T> model) {
		this.model = (Journal<T>) model;// model;//new Journal<>(new
										// JournalObserver());
	...
	public boolean commandParse(String command, String... commandArgs) throws IOException {
		...
		switch (command) {
			...
			case "stop": run = false;
				...
				break;
			default:
				System.out.println("Вы ввели неверную команду. Попробуйте заново или воспользуйтесь справкой 'help'");
		}
		return run;
	}
	...
	private boolean add() {
		try {
			...
			do { // лучше заюзать счетчик по строкам и выкидывать если более
					// 3-х, иначе это может быть бесконечно
				command = reader.readLine();

				if (!command.equals("ok")) // здесь проверка на 'ок', либо > 3
											// строк
					commandArgs.add(command);
				else {
					// здесь должны вызвать подходящий метод (в нашем случае
					// только при добавлении задач множество вариаций)
					addTask(commandArgs);
					break;
				}
			} while (true);
	...
	private boolean addTask(List<String> commandArgs) {
		...
		if (commandArgs != null) {
			...
			case 0:
				run = controller.addTask();
				break;
			case 1:
				run = controller.addTask(commandArgs.get(0));
				break;
			// тут нужно как то определять какая это добавлялка: с датой или
			// описанием
			case 2:
				run = controller.addTask(commandArgs.get(0), commandArgs.get(1));
				break;
			case 3:
				run = controller.addTask(commandArgs.get(0), commandArgs.get(1), Date.valueOf(commandArgs.get(2)));
				break;
	...
	private boolean edit(){
		...
		try{
			BufferedReader reader = new BufferedReader(new InputStreamReader(System.in));
			System.out.print("Введите название задачи, которую хотите отредактировать: ");
			...
			System.out.println("Задача '"+task.getTitle()+"', отредактируем ее");
			...
			Date editDate =(date != null && !date.isEmpty())? Date.valueOf(date): null;  
			
			result = controller.editTask(title, editTitle, editDescription, editDate); 
			System.out.println((result)? "Задача успешно отредактирована": "Ошибка при редактировании задачи");
			return result;
	...
	private void showAll() {
		// пока так
		System.out.println((controller.getTasks().isEmpty()) ? "Список задач пуст" : controller.getTasks());
	}
	...
	private void record() {
		...
		try {
			...
			// запишется в любом случае, даже если NPE не пройдет =) p.s. скорее
			// всего)
	...
	private void read() {
		...
		try {
			...
			model.replaceTasks((List<? extends T>) controller.readJournal(fileName).getTasks());// !
																								// проблема
																								// ClassCastException
	...
	private void help() {
		try (BufferedReader reader = new BufferedReader(new FileReader("storage/help.txt"))) {
			String s;
			do {
				s = reader.readLine();
				if (s == null)
